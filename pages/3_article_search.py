...
if keyword: filtered = filtered[(filtered["title"] + " " + filtered["summary"]).str.contains(re.escape(keyword), case=False, na=False)]
...
st.dataframe(filtered[["게시일", "professor_name", "publisher", "title", "mention_type", "topic", "relevance_score", "review_status", "source", "url"]], hide_index=True, use_container_width=True, column_config={"professor_name": "교수", "publisher": "언론사", "title": "기사 제목", "mention_type": "유형", "topic": "주제", "relevance_score": st.column_config.ProgressColumn("관련도", min_value=0, max_value=100), "review_status": "검토 상태", "source": "수집 출처", "url": st.column_config.LinkColumn("원문")})
